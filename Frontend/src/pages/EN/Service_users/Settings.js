...
import './Settings.css';
import { FaHome, FaUsers, FaCog, FaSignOutAlt, FaBell, FaTrash } from 'react-icons/fa';
...
function Settings() {
    ...
    useEffect(() => {
        setupAxiosAuth(); // ตั้งค่า token เมื่อ component โหลด
    ...
    const fetchWalletBalance = async (userId) => {
        try {
            ...
            if (error.response && error.response.status === 401) {
                navigate('/login'); // Redirect ถ้า token ไม่ถูกต้อง
            }
        }
    };
                ...
                <div className="container">
                    ...
                    <div className="collapse navbar-collapse" id="navbarNav">
                        <ul className="navbar-nav me-auto mb-2 mb-lg-0">
                            ...
                            <li className="nav-item">
                                <a className="nav-link active" href="/Settings"><FaCog className="me-2" /> {language === 'EN' ? 'Settings' : 'ตั้งค่า'}</a>
                            </li>
                        </ul>
